...
namespace GamePlay.Event
{
    ...
    public class ReinforceTentacle : Event
    {
        public ReinforceTentacle()
        {
            //在来源战区随机部署X个触手（CR Y）
            //从数据库读取属性，id名不能错
            EventDataBase.Instance().GetEventProperty("ReinforceTentacle", this);
            //实例化该事件的 触发条件 和 效果
        ...
        void selfAction()
        {
            ...
            if (source_type_flag == 2)
            {
                ...
                if (_area._battleAreaSate == BattleMap.BattleAreaState.Enmey)
                {
                    switch (strenth)
                    {
                        case 1: SummonMonster_in_Area(this.Source, this.amount, "Tentacle_1"); break;
                        case 2: SummonMonster_in_Area(this.Source, this.amount, "Tentacle_2"); break;
                        case 3: SummonMonster_in_Area(this.Source, this.amount, "Tentacle_3"); break;
                        default: break;
                    }
                }

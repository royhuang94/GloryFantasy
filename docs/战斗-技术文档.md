...
- [3. 类说明](#3-%E7%B1%BB%E8%AF%B4%E6%98%8E)
  ...
  - [3.2. 遭遇及地图](#32-%E9%81%AD%E9%81%87%E5%8F%8A%E5%9C%B0%E5%9B%BE)
    ...
    - [3.2.2. BattaleMap](#322-BattaleMap)
      - [重要成员](#%E9%87%8D%E8%A6%81%E6%88%90%E5%91%98)
    - [BattleAreaData](#BattleAreaData)
      - [重要成员](#%E9%87%8D%E8%A6%81%E6%88%90%E5%91%98-1)
...
概括来说，把DataOfThisEncounter和Player合并到这里就差不多了。
...
+ BattleAreaData
  + 战区实例的管理类
+ DebuffBattleMapBlock
  + 异常地形的地图侧接口
+ DrawBattleArea
+ BattleMapBlock

BattleMap持有以下重要成员及成员方法

+ public void InitMap()		
  + 初始战斗地图相关
+ public void RestarInitMap(string encouterID, Mediator.Deck deck) 	
  + 重新初始战斗地图，用于遭遇选择器直接在战斗场景初始不同的地图
...
+ public Unit GetUnitsOnMapBlock(Vector3 vector)	
  + 返回给定坐标上的单位，坐标不合法会返回null, 其他依据实际情况返回值
...
+ public Dictionary<int, List<Vector2>> BattleAreaDic; 
  + 战区id及对应的该战区中的所有地图块坐标
+ public Dictionary<int, BattleArea> battleAreas 
  + 战区id及对应的战区对象
+ public void InitBattleArea(string encounterId)
  + 初始战区对象，存入battleAreas 
